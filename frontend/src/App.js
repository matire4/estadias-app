...
function App() {
  ...
  const fetchEstadias = async (filters = {}) => {
    try {
      setLoading(true)
      let url = "http://localhost:3001/estadias"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingId ? `http://localhost:3001/estadias/${editingId}` : "http://localhost:3001/estadias"
  ...
  const handleDelete = async (id) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar esta estadía?")) {
      try {
        const response = await fetch(`http://localhost:3001/estadias/${id}`, {
          method: "DELETE",
        })
